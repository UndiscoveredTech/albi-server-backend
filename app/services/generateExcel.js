...
totalObject = {
  ...
  calculations.forEach((user,index) => {
    ...
    //generate sheet for every employee
    generateSingleWorksheet(workbook,user,index+1);
  });
  ...
  generateSingleWorksheet = (workbook, user,index) => {
    const worksheetSingle = workbook.addWorksheet(user.emploeyeeName + "-" + index ); // New Worksheet for single user
    worksheetSingle.columns = worksheetColumns;

    worksheetSingle.addRow(user); // Add data in worksheet
    worksheetSingle.columns.forEach(column => {
